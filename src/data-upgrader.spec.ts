...
describe('Data upgrade process', () => {
    it('runs upgrades', async () => {
        ...
        mockQueryExecutor
            ...
            .match(({ upgradeName }) => {
                if (
                    updateResult.applied_upgrades &&
                    updateResult.applied_upgrades.includes(upgradeName)
                ) {
                    return []
                }
        ...
        // For cleanups, covered in another test
        mockQueryExecutor.mock(getTestWithUpgradesApplied).match(() => [])
        mockQueryExecutor.mock(upgradeTestQuery).match(args => {
            updateResult = { ...updateResult, ...args.updates }
        })

        const upgrader = createDataUpgrader(
            ['Test'],
            {
                upgradesToRun: {
            ...
            {
                // Continue right away
                setTimeoutOverride: cb => setTimeout(cb, 10),
            },
        )

        await upgrader.run(() => mockQueryExecutor, consoleLogger(), {})
    ...
    it('runs async upgrades', async () => {
        ...
        expect(updateResult!.id).toBe('test')
        expect(updateResult!.applied_upgrades).toEqual([
            asyncDoubleDataUpgrade.upgradeName,
        ])
        expect(updateResult!.data).toEqual(2)
    })
        ...
        mockQueryExecutor.mock(getTestWithUpgradesApplied).match(() =>
            updateResult
                ...
                : [
                      {
                          id: 'test',
                          applied_upgrades: [doubleDataUpgrade.upgradeName],
                          data: 2,
                      },
                  ],
        )
