 ...
 *
 */
export function createDataUpgrader<
...
>(
    ...
    options: {
        upgradesToRun: {
            [tableName in TableNames]: Array<
                DataUpgrade<any, TableNames, Services>
            >
        }
    ...
    const remainingUpgrades: { [tableName in TableNames]: string[] } = {
        ...mapObject(
            options.upgradesToRun,
            (upgrades: Array<DataUpgrade<any, TableNames, Services>>) =>
        ...
        async run(getQueryExecutor, logger, services) {
            ...
            for (const tableName of tableNames) {
                await runDataCleanupsForTable(
                    ...
                    {
                        batchSize:
                            upgradeOptions.cleanupBatchSize ||
...
>(
    tableName: TableNames,
    upgradesToRun: Array<DataUpgrade<Schema, TableNames, UpgradeServices>>,
    ...
    for (const upgrade of upgradesToRun) {
        ...
        try {
            ...
            do {
                ...
                const shouldRunDataUpgrades =
                    !options.enabled || options.enabled()

                if (!shouldRunDataUpgrades) {
                    logger.debug(
                        { tableName },
                        'Skipping data upgrades due to run-data-upgrades toggle',
                    )
                    ...
                    if (
                        dataUpgraderState.currentlyRunningUpgrade.tableName !==
                            tableName ||
                        dataUpgraderState.currentlyRunningUpgrade
                            .upgradeName !== upgradeName
                    ) {
                        logger.info(
                            { tableName, upgradeName },
                            'Starting data upgrade',
                        )
                    }
                    ...
                    logger.debug(
                        { tableName, upgradeName },
                        'Performing data upgrade batch',
                    )

                    ranUpgrades = false
                    const queryExecutor = getQueryExecutor()

                    const batchToUpgrade = await queryExecutor.execute(
                        getWithoutUpgradesApplied,
                        {
                            upgradeName,
                            limit:
                                batchSizeOverrides[upgradeName] ||
                                options.batchSize,
                        },
                    )
                    ...
                    for (const schema of batchToUpgrade) {
                        try {
                            await queryExecutor.unitOfWork(
                                async transaction => {
                                    let updates: Updates = {}
                            ...
                            numberOfErrors++
                            logger.error(
                                { err: error, tableName, upgradeName, schema },
                                'Failed to upgrade item',
                            )
                        }
                    }
                ...
                timeoutDuration = getTimeoutDuration(
                    ...
                    timeoutDuration,
                    shouldRunDataUpgrades,
                    logger,
                )
                await new Promise(resolve =>
                    (options.setTimeoutOverride || setTimeout)(
                        resolve,
                        timeoutDuration,
                    ),
                )
            ...
            // eslint-disable-next-line require-atomic-updates
            dataUpgraderState.remainingUpgrades[
                tableName
            ] = dataUpgraderState.remainingUpgrades[tableName].filter(
                remainingUpgrade => remainingUpgrade !== upgradeName,
            )
            logger.info(
                { tableName, upgradeName, dataUpgraderState },
                'Done performing upgrade',
            )
        } catch (err) {
            logger.error(
                { err, upgrade: tableName, upgradeName },
                'Failed to perform data upgrade',
            )
        }
    }
...
>(
    ...
    for (const toCleanup of upgradesToCleanup) {
        ...
        do {
            ...
            if (!shouldRunDataUpgrades) {
                ...
                await getQueryExecutor().unitOfWork(async queryExecutor => {
                    const pubs = await queryExecutor.execute(
                        getWithUpgradesApplied,
                        {
                    ...
                    for (const pub of pubs) {
                        ...
                        dataUpgraderState.remainingCleanups[tableName].forEach(
                            upgradeName => {
                                appliedUpgrades = appliedUpgrades.filter(
                        ...
                        logger.debug(
                            { tableName, id: pub.id, toCleanup },
                            'Persisting upgrade cleanup',
                        )
        ...
        dataUpgraderState.remainingCleanups[
            tableName
        ] = dataUpgraderState.remainingCleanups[tableName].filter(
            remainingCleanup => remainingCleanup !== toCleanup,
        )
        logger.info(
            { domainType: tableName, cleanup: toCleanup },
            'Done cleaning up upgrade',
        )
    }
...
function getTimeoutDuration(
    ...
    if (!shouldRunDataUpgrades) {
        ...
        logger.debug(
            { timeoutDuration },
            `Not running data upgrades so long sleep`,
        )
    ...
    if (timeoutDuration > oneMinute) {
        logger.warn(
            { timeoutDuration: oneMinute, previousBatchDuration: duration },
            `Data upgrade sleep capped to 60s`,
        )
        return oneMinute
    }
